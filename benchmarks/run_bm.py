...
def bm_check(version, taken, runs=1):
    if version not in {"python", "swi", "xsb", "clingo", "ortools"}:
        return 0
    times = []
    ...
    for i in range(runs):
        match version:
            ...
            case "swi":
                times.append(run_prolog(taken, "swi", return_timing=True)["prolog_eval_s"])
            case "xsb":
                times.append(run_prolog(taken, "xsb", return_timing=True)["prolog_eval_s"])
            case "ortools":
...
def bm_plan(version, taken, runs=1):
    if version not in {"clingo", "ortools"}:
        return 0
    times = []
...
def write_to_file(dir_name, results, cases_in_order):
    ...
    with open(out_dir / filename, "w") as f:
        ...
        for i, case in enumerate(cases_in_order):
            row = [
                f"{(results.get(case, {}).get(version, 0)):.9f}"
                for version in header
                if version in results.get(case, {}).keys()
            ]
            print(row)
