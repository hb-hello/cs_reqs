...
Course = namedtuple(
    "Course",
    ["credits", "prereq", "coreq", "anti_req", "pre_or_coreq", "allreqs"],
...
class Coreq(Requirement):
    pass


class AntiReq(Requirement):
    pass
...
def _rewrite_for_allreqs(expr):
    ...
    if isinstance(expr, TakenReq):
        return TakenId(*expr.arguments)
...
def _filter_for_allreqs(expr, valid_ids):
    ...
    if isinstance(expr, (And, Or)):
        ...
        if len(ops) == 1:
            return ops[0]
        return type(expr)(*ops)
    if isinstance(expr, _keep_as_is):
        return expr
...
def _has_coregister(expr):
    if isinstance(expr, Coregister):
        return True
    if isinstance(expr, (And, Or)):
        return any(_has_coregister(op) for op in expr.operands)
    return False
...
def _wrap_as_prereqs(expr):
    ...
    if isinstance(expr, _keep_as_is):
        return expr
    if isinstance(expr, Coregister):
        return Coreq(TakenId(*expr.arguments))
    if not _has_coregister(expr):
        return Prereq(expr)
...
def _build_course_allreqs(kc, valid_ids):
    def prep(field):
        return _filter_for_allreqs(_rewrite_for_allreqs(field), valid_ids)
    ...
    if prereq is not None:
        parts.append(_wrap_as_prereqs(prereq))
    if coreq is not None:
        parts.append(_wrap_as_coreqs(coreq))
    if poc is not None:
        alts = [
            x for x in (_wrap_as_prereqs(poc), _wrap_as_coreqs(poc)) if x is not None
        ]
        if alts:
            parts.append(Or(*alts) if len(alts) > 1 else alts[0])
    if anti is not None:
...
def upper_division(cid):
    return int(cid[4:]) >= 300


# provides range of (year, semester) tuples
def semester_range(start, end_or_count=MAX_SEMS_ALLOWED):
    y, s = start
    if end_or_count is None:
        end_or_count = MAX_SEMS_ALLOWED
...
def _rewrite_req_ids(expr):
    ...
    if type(expr) is Passed:  # normalize base Passed to a typed subclass
        ...
        return cls(expr.course_id) if cls else expr
    if isinstance(expr, (UnsupportedRequirement, Permission, Major, Standing)):
        return None
    return expr
...
_kb_path = os.path.join(
    os.path.dirname(__file__), "..", "course_kb", "kb_cse_degree.json"
)
print("processing kb.")
...
def _dep_tree_from_expr(expr, seen):
    if isinstance(expr, And):
        parts = [
            p for p in (_dep_tree_from_expr(op, seen) for op in expr.operands) if p
        ]
        if not parts:
            return ""
...
def _option_count_from_expr(expr, prereq_counts, seen, ignore_course=None):
    ...
    if isinstance(expr, Requirement):
        ...
        return _option_count_from_expr(
            dep_expr, prereq_counts, seen | {dep}, ignore_course
        )
    return 1
...
def options_opened_up(cid, dependents, prereq_counts):
    ...
    return sum(
        prerequisite_options(dep, prereq_counts, ignore_course=cid)
...
def top_requisite_courses(n=10, kb_path=None):
    rows = [
        (cid, prereq_n, coreq_n, total)
        for cid, (prereq_n, coreq_n, total) in requisite_counts_by_course(
            kb_path
        ).items()
    ]
...
def main():
    ...
    for cid in FULL:
        ...
        if prereq_n is None:
            print(
                f"{cid:8} | not found | not found | not found | not found | not found"
            )
            rows.append(
                {
                    "course_id": cid,
        ...
        print(
            f"{cid:8} | {prereq_n:6} | {options:7} | {dependents_count:10} | {opened:17} | {tree}"
        )
        rows.append(
            {
                "course_id": cid,
