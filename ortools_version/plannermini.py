...
def print_schedule(planned,grades,credits_fn):
    ...
    for s in sorted(by_sem):
        yr,sn=s;total=sum(credits_fn(c) for c in by_sem[s])
...
def plan_courses(already_taken,*student_reqs,must_exclude=set(),must_include=set(),check=False,starting_semester=(1,1),course_offered_terms=None,debug_print=False):
    ...
    for cid,h in history_ids.items(): or_model[Grade(cid)]=h.grade;or_model[Taken(cid)]=1;or_model[Semester(cid)]=h.when
    for cid in to_plan_from|(must_exclude-history_ids.keys()): or_model.iff(Taken(cid),Grade(cid))
    if not check:
        for cid in to_plan_from:
            or_model.iff(Taken(cid),Semester(cid));offered_terms=course_offered_terms.get(cid)
            if offered_terms:
                offered_sems=[sem for sem in Semester.domain[1:] if sem>=starting_semester and SEM_NAMES[sem[1]] in offered_terms]
                if offered_sems: or_model.implies(Taken(cid),Or(*[or_model.exactly(Semester(cid),sem) for sem in offered_sems]))
                else: or_model[Taken(cid)]=0
            else: or_model.implies(Taken(cid),or_model.at_least(Semester(cid),starting_semester))
        for cid in must_exclude-history_ids.keys(): or_model[Taken(cid)]=0
        for cid in must_include: or_model[Taken(cid)]=1
    or_model[Passed]=lambda c,g:or_model.at_least(Grade(c),g)
    credits=lambda c:history_ids[c].credits if c in history_ids else catalog[c].credits
    reqs={};witnesses={}
    prog={'CSE 114','CSE 214','CSE 216'};prog2={'CSE 160','CSE 161','CSE 260','CSE 261'};dmath={'CSE 215'};dmath2={'CSE 150'};sys={'CSE 220'}
    ...
    theory={'CSE 303'};theory2={'CSE 350'};algo={'CSE 373'};algo2={'CSE 385'};other={'CSE 310','CSE 316','CSE 320','CSE 416'}
    ...
    reqs["elect"]=or_model.at_least(sum(or_model.resolve(Passed(c)) for c in electives),4)
    calc={'AMS 151','AMS 161'};calc2={'MAT 125','MAT 126','MAT 127'};calc3={'MAT 131','MAT 132'}
    reqs["calc"]=Or(And(*map(Passed,calc)),And(*map(Passed,calc2)),And(*map(Passed,calc3)))
    alg={'MAT 211'};alg2={'AMS 210'}
    reqs["alg"]=Or(And(*map(Passed,alg)),And(*map(Passed,alg2)))
    fmath={'AMS 301'};sta={'AMS 310'};sta2={'AMS 311'}
    ...
    for cid in sci_ids: or_model.implies(SciSubset(cid),Taken(cid))
    ...
    reqs["sci"]=And(reqs["sci_combo"],or_model.at_least(sci_subset_credits,9),or_model.at_least(sci_subset_grade_points,200*sci_subset_credits))
    ...
    items123_courses={h.id for h in intro_courses|adv_courses|electives if h.where=='SB'}
    items23_courses=items123_courses - intro_courses
    reqs['credits_at_SB']=And(or_model.at_least(sum(or_model[Passed(c)]*credits(c) for c in items123_courses),24),or_model.at_least(sum(or_model[Passed(c)]*credits(c) for c in items23_courses),18))
    grades={h.id:h.grade for h in already_taken}
    req_vars={name:or_model.resolve(expr) for name,expr in reqs.items()}
    if check:
        for cid in to_plan_from: or_model[Taken(cid)]=0
        or_model.maximize(sum(req_vars.values()))
    else:
        for v in req_vars.values(): or_model.require(v)
        new_courses=sum(or_model[Taken(cid)] for cid in to_plan_from)
        prereqs={cid:c.prereq for cid,c in catalog.items() if c.prereq}
        for cid,expr in prereqs.items():
            if cid in excluded: continue
            or_model.implies(Taken(cid),expr)
            for p in get_courses(expr): or_model.implies(Taken(cid),or_model[Semester(cid)]>or_model[Semester(p)])
        for cid,c in catalog.items():
            if not c.coreq or cid in excluded: continue
            or_model.implies(Taken(cid),c.coreq)
            for p in get_courses(c.coreq): or_model.implies(Taken(cid),or_model[Semester(p)]<=or_model[Semester(cid)])
        for cid,c in catalog.items():
            if not c.anti_req or cid in excluded: continue
            or_model.forbids(Taken(cid),c.anti_req)
        ...
        grade_sum=sum(or_model.apply(Grade(cid),lambda g:int(grade_points[g]*100),iff=Taken(cid)) for cid in to_plan_from)
    ...
    if solution.obj is None: print("No solution:",solution.status);return {}
