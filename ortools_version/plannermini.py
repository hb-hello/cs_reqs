...
def plan_courses(taken,*student_reqs,must_exclude=set(),must_include=set(),check=False,starting_semester=(1,1),ending_semester=None,course_offered_terms=None,debug_print=False):
    ...
    if not check:
        for cid in to_plan_from:
            ...
            if offered_terms:
                ...
                if offered_sems: or_model.implies(TakenId(cid),Or(*[or_model.exactly(Semester(cid),sem) for sem in offered_sems]))
                else: or_model[TakenId(cid)]=0
            else: or_model.implies(TakenId(cid),or_model.at_least(Semester(cid),starting_semester))
    ...
    reqs['credits_at_SB']=And(or_model.at_least(sum(or_model[PassedId(c)]*credits(c) for c in items123_courses),24),or_model.at_least(sum(or_model[PassedId(c)]*credits(c) for c in items23_courses),18))
    ...
    if check:
        ...
        or_model[Prereq]=lambda cid,req: or_model.resolve(And(req,or_model.at_least(or_model[Semester(cid)]-or_model[Semester(course_of(req))],1)))
        or_model[Coreq]=lambda cid,req: or_model.resolve(And(req,or_model.exactly(or_model[Semester(cid)]-or_model[Semester(course_of(req))],0)))
