...
## predicate to represent grade that student has achieved in a course
class Sem(ReqWithDomain):
    pass


## predicate to represent grade that student has achieved in a course
class Grade(ReqWithDomain):
    pass


## credits IntVar for variable-credit courses
class Cr(ReqWithDomain):
    pass
...
def plan_courses(
    ...
    *student_reqs,
    ...
    if end_sem is None:
        end_sem = list(semester_range(start_sem, end_sem))[-1]
    start_sem, end_sem = int_sem(start_sem, base), int_sem(end_sem, base)

    history = {h.id: h for h in taken}
    taken_ids = {h.id for h in taken}

    for cid, course in catalog.items():
        cr = history[cid].credits if cid in history else course.credits
        lo, hi = cr if isinstance(cr, tuple) else (cr, cr)
        m.set_domain(Cr(cid), list(range(lo, hi + 1)))
    for cid, h in history.items():
        if cid in catalog:
            m[Cr(cid)] = h.credits

    to_plan = catalog.keys() - (taken_ids | must_exclude)
    to_plan &= course_offered_terms.keys()  # only courses with offered semesters will be planned
    ...
    def c_or_higher(course):
        return m.resolve(m[Grade(course)] >= int_grade["C"])

    def b_or_higher(course):
        return m.resolve(m[Grade(course)] >= int_grade["B"])

    def b_plus_or_higher(course):
        return m.resolve(m[Grade(course)] >= int_grade["B+"])

    def d_or_higher(course):
        return m.resolve(m[Grade(course)] >= int_grade["D"])
    ...
    reqs["intro"] = m.add(
        And(
            Or(And(*map(C_or_higher, prog)), And(*map(C_or_higher, prog2))),
    ...
    reqs["adv"] = m.add(
        And(
            Or(And(*map(C_or_higher, theory)), And(*map(C_or_higher, theory2))),
            Or(And(*map(C_or_higher, algo)), And(*map(C_or_higher, algo2))),
            And(*map(C_or_higher, other)),
        )
    )
    ...
    def cat_credits(c):
        cr = catalog[c].credits
        return cr[-1] if isinstance(cr, tuple) else cr

    electives = {
        c
        for c in catalog
        if c[:3] == "CSE" and upper_division(c) and cat_credits(c) >= 3 and c not in adv_courses | elect_exclude
    }

    reqs["elect"] = m.add(sum(m[C_or_higher(c)] for c in electives) >= 4, "elect")
    ...
    reqs["calc"] = m.add(
        Or(
            And(*map(C_or_higher, calc)),
            And(*map(C_or_higher, calc2)),
            And(*map(C_or_higher, calc3)),
        )
    )
    ...
    alg2 = {"AMS 210"}
    reqs["alg"] = m.add(
        Or(And(*map(C_or_higher, alg)), And(*map(C_or_higher, alg2)))
    )  # wrap in And just in case courses are added to the sets
    ...
    reqs["sta"] = m.add(
        And(
            And(*map(C_or_higher, fmath)),
            Or(And(*map(C_or_higher, sta)), And(*map(C_or_higher, sta2))),
        )
    )
    ...
    sci_grade_points = sum(m.product(m.select(points, Grade(cid)), m[Cr(cid)]) for cid in sci_ids)
    # count each sci course once (for 9-credit min and GPA denominator)
    sci_credits = sum(m.product(m[TakenId(cid)], m[Cr(cid)]) for cid in sci_ids)
    ...
    reqs["sci"] = m.add(And(sci_combo, sci_credits > 9, sci_grade_points >= 200 * sci_credits), "sci")

    # 9. Professional Ethics
    ethics_courses = {"CSE 312"}
    reqs["ethics"] = m.add(And(*map(C_or_higher, ethics_courses)))

    # 10. Upper-Division Writing Requirement
    writing_courses = {"CSE 300"}
    reqs["writing"] = m.add(And(*map(C_or_higher, writing_courses)))
    ...
    reqs["credits_at_SB"] = m.add(
        And(
            sum(m.product(m[C_or_higher(c)], m[Cr(c)]) for c in items123_courses) >= 24,
            sum(m.product(m[C_or_higher(c)], m[Cr(c)]) for c in items23_courses) >= 18,
        )
    )
    ...
    if check:
        ...
        for cid in to_plan:
            if not catalog[cid].allreqs:
                continue
            sat, cid_cond = m.traverse(catalog[cid].allreqs, negated=(AntiReq,))
        ...
        for sem in range(start_sem, end_sem + 1):
            sem_credits = [
                m.product(m[Cr(cid)], m.eq(Sem(cid), sem)) for cid in offered_sems if sem in offered_sems[cid]
            ]
            if sem_credits:
                m.add(sum(sem_credits) <= CREDIT_LIMIT)
        ...
        grade_sum = sum(m.select(points, Grade(cid)) for cid in to_plan)
    ...
    checked = {name: (bool(sol.value(v)), sorted({cid_of(leaf) for leaf in sol.chosen(v)})) for name, v in reqs.items()}

    items123_cr = sum(sol.value(m[Cr(c)]) for c in items123_courses if sol.value(c_or_higher(c)))
    items23_cr = sum(sol.value(m[Cr(c)]) for c in items23_courses if sol.value(c_or_higher(c)))
    ...
    if not check:
        ...
        if debug_print:
            print_schedule(planned, grades, lambda c: sol.value(m[Cr(c)]))
